...
def create_project_route(project_name: str, db: Session = Depends(get_db)) -> ProjectData:
    ...
    return ProjectData(data=ProjectEntry(project_name=new_project.project_name, project_id=new_project.project_id, config_id=new_project.config_id))
...
def get_projects_route(db: Session = Depends(get_db)) -> ProjectsData:
    ...
    return ProjectsData(
        data=[ProjectEntry(project_name=project.project_name, project_id=project.project_id, config_id=project.config_id) for project in projects],
        length=len(projects),
    )
...
def get_project_route(project_id: int, db: Session = Depends(get_db)) -> ProjectData:
    ...
    return ProjectData(data=ProjectEntry(project_name=project.project_name, project_id=project.project_id, config_id=project.config_id))


@router.put("/{project_id}/")
def update_project_route(project_id: int, project_name: str, db: Session = Depends(get_db)) -> ProjectData:
    project = db.query(Project).filter(Project.project_id == project_id).first()
    if not project:
        raise HTTPException(status_code=404, detail=f"Project '{project_id}' does not exist.")
...
def set_project_config_route(project_id: int, config_id: int, db: Session = Depends(get_db)) -> ProjectData:
